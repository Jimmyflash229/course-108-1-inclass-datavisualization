...
#####
NROW(unique(dataList$sub$工作領域)

dataList$b0 %>% group_by(
  受傷部位,工作領域
) %>% summarise(
  受傷部位次數=sum(受傷部位次數)/NROW(unique(dataList$sub$工作領域))) ->dataList$test
dataList$test[-(1:4),] -> dataList$test
dataList$test$受傷部位<- as.factor(dataList$test$受傷部位)
